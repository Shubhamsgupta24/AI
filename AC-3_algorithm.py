    ...
    for color_option in curr_domain:
        ...
        for neighbor in search_space[curr]:
            if color[neighbor] is None and color_option not in domain[neighbor]:
                domain[neighbor].add(color_option)
        color[curr] = None

def AC3(search_space, domain):
    queue = deque([(i, j) for i in search_space for j in search_space[i]])
    ...
    while queue:
        ...
        for i in temp:
            check = not all(i != j for j in domain[y])
            if check:
                domain[x].remove(i)
                change = True
        ...
        if change:
            for i in search_space:
                if i != y and x in search_space[i]:
                    queue.append((i, x))
...
if __name__ == "__main__":
    ...
    domain = AC3(search_space, domain)
    ...
    if result:
        ...
        print("Assignment of colors not possible")
